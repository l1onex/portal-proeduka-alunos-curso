...
type Props = {
  ...
  disabled: boolean;
  inputClass: string;
  onChange: (next: { uf: string | null; cidade: string | null }) => void;
};

/**
 * Bloco "Estado + Cidade" usado no endereço.
 * Igual ao `NaturalidadeFields`, mas com as labels "Estado" e "Cidade"
 * (em vez de "UF" e "Município"), e dentro do mesmo "quadradinho".
 */
export function EnderecoEstadoCidadeFields({
  uf,
  cidade,
  disabled,
  inputClass,
  ...
  useEffect(() => {
    ...
    void fetchIbgeMunicipiosNomes(sigla)
      ...
      .catch(() => {
        if (!cancelled) {
          setMunicipios([]);
          setErr("Não foi possível carregar as cidades. Tente de novo.");
        }
      })
      .finally(() => {
  ...
  return (
    <div className="mt-1 space-y-2">
      <div className="grid gap-3 sm:grid-cols-[minmax(5.5rem,7rem)_1fr] sm:items-end">
        <div>
          ...
          <select
            ...
            disabled={disabled}
            onChange={(e) =>
              onChange({ uf: e.target.value || null, cidade: null })
            }
          >
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => {
              ...
              onChange({
                uf: ufVal.length === 2 ? ufVal : null,
                cidade: v ? v.toUpperCase() : null,
              });
            }}
          >
            <option value="">
      ...
      {err ? (
        <p className="text-xs font-medium text-amber-800">{err}</p>
      ) : null}
    </div>
  );
}